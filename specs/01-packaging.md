...
* **구성 요소 및 권한:**
  ...
  * `mudfish-dns-cli.exe`: CLI 환경에서 제어하기 위한 명령줄 도구.
...
* **패키징 및 배포:** Mac App Store 전용 배포
  ...
  * (참고: CLI 도구인 `mudfish-dns-cli`는 App Store 샌드박스 정책상 번들에 포함하여 기본
    PATH에 노출하기 어려우므로, macOS용 CLI는 Homebrew 등의 별도 채널을 통해 독립적으로
    배포하거나 배포에서 제외합니다.)
...
* **구성 요소 및 권한:**
  * 바이너리 배포: 바이너리 파일들은 `/opt/mudfish-dns/bin/` 에 위치합니다.
    (`mudfish-dns-core`, `mudfish-dns-ui`, `mudfish-dns-service`,
    `mudfish-dns-cli`)
