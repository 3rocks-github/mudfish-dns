...
- **`crates/core`**: 업스트림 서버로의 라우팅 정책(UDP, TCP, DoH 등), DNS
  ...
  단위 테스트.
- **`crates/service`**: OS별 네트워크 구성(Windows: WFP, Linux:
  eBPF/iptables, macOS/iOS: NetworkExtension, Android: VpnService) 변경
  명령 및 데몬 프로세스 수명 주기 제어 로직. (OS 권한이나 시스템 상태에
  종속되는 테스트는 CI 환경에서 적절히 스킵하거나 별도 환경 구축 필요)
